...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--gc",
        action="store_true",
        help="Run a one-shot maintenance pass. Retention is FOUR-TIER: run "
        ...
        "directory identity still matches the GC plan, after confirming via lsof "
        "that no live process has its working dir inside — workspaces where that "
        "check cannot answer (lsof unavailable or errored) are declined and kept, "
        "listed on stdout. Orphaned reviewer/producer subprocesses whose working "
        "dir is INSIDE a workspace being removed are reaped before the rmtree. "
        "Runs also auto-prune their transcripts at the start of "
