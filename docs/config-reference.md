...
| `worktree_max_age_days` | int ≥ 0 | `14` | Days a run's **ownership-proven actor workspaces** are kept, after which they are removed — including while the run is still resume-eligible. A workspace is also removed when its run directory is gone, its ownership record names this repository, AND trusted repo state hard-links that exact record file (gone-run orphan reclamation; both factors required — record bytes alone are not proof). Recordless Syncade-shaped workspaces are never removed automatically and `syncade --gc` reports them for manual cleanup, including when matching repo-local run artifacts still exist. A root that cannot be inspected is also reported when its name matches repo-local run artifacts; make it inspectable and rerun GC. The report says `size unknown` if unreadable content prevents an exact total. An inspectable malformed or foreign record authorizes no deletion and is excluded from the recordless report rather than mislabeled. Age alone never authorizes removal either: a workspace whose live-process check could not ANSWER is `declined` and kept — which is every workspace when `lsof` is not installed. One is never removed because its transcripts became prunable, so a young export/worktree you are still inspecting survives a busy week of runs. Separate from `max_age_days`, which gates transcripts only; reviewer exports and linked worktrees are reconstructible from recorded snapshot state. `0` disables the age-based bound; orphan reclamation continues regardless. |
